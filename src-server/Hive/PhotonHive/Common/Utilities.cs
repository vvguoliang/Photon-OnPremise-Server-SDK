    ...
    public static class Utilities
    {
        ...
        public static void ConvertAs3WellKnownPropertyKeys(Hashtable gameProps, Hashtable actorProps)
        {
            ...
            if (gameProps != null && gameProps.Count > 0)
            {
                ...
                if (gameProps.ContainsKey(amf3PlayerTTLPropertyKey))
                {
                    gameProps[(byte)GameParameter.PlayerTTL] = gameProps[amf3PlayerTTLPropertyKey];
                    gameProps.Remove(amf3PlayerTTLPropertyKey);
                }

                // well known property "empty room ttl"
                if (gameProps.ContainsKey(amf3EmptyRoomTTLPropertyKey))
                {
                    gameProps[(byte)GameParameter.EmptyRoomTTL] = gameProps[amf3EmptyRoomTTLPropertyKey];
                    gameProps.Remove(amf3EmptyRoomTTLPropertyKey);
                }
            }
        ...
        public static void ConvertAs3WellKnownPropertyKeys(IList gamePropertyKeys, IList actorPropertyKeys)
        {
            ...
            if (gamePropertyKeys != null && gamePropertyKeys.Count > 0)
            {
                ...
                // well known property "player ttl"
                if (gamePropertyKeys.Contains(amf3PlayerTTLPropertyKey))
                {
                    gamePropertyKeys.Remove(amf3PlayerTTLPropertyKey);
                    gamePropertyKeys.Add((byte)GameParameter.PlayerTTL);
                }

                // well known property "empty room ttl"
                if (gamePropertyKeys.Contains(amf3EmptyRoomTTLPropertyKey))
                {
                    gamePropertyKeys.Remove(amf3EmptyRoomTTLPropertyKey);
                    gamePropertyKeys.Add((byte)GameParameter.EmptyRoomTTL);
                }
            }
