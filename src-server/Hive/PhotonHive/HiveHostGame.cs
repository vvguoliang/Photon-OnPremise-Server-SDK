...
namespace Photon.Hive
{
    ...
    public class HiveHostGame : HiveGame, IPluginHost, IHttpRequestQueueCounters, IUnknownTypeMapper, IPluginHttpUtilClient
    {
        ...
        public IPluginInstance PluginInstance { get { return this.pluginWrapper.PluginInstance; } }
        ...
        {
            get { return this.ExecutionFiber.IsPaused; }
        }

        public bool IsSyspended
        {
            ...
            Photon.Hive.Plugin.SendParameters sendParameters)
        {
            ...
            string errorMsg;
            var actors = this.GetActorsFromTraget(target, senderActor, targetGroup, out errorMsg);
            if (actors == null)
            {
            ...
            byte cacheOp, bool updateEventCache, int senderActor, IEnumerable<Actor> actors, Plugin.SendParameters sendParameters)
        {
            if (updateEventCache && cacheOp != (byte)CacheOperation.DoNotCache)
            {
                ...
                {
                    msg = string.Format("Unsupported value {0} for cacheOp. Use method ExecuteCacheOperation", cacheOp);
                    Log.Error(msg);
        ...
        public bool ExecuteCacheOperation(CacheOp operation, out string errorMsg)
        {
            if (operation.ActorNr > 0)
            {
                ...
                if (actor == null)
                {
                    errorMsg = string.Format("Invalid senderActor={0} specified. Number may be 0 or existing", operation.ActorNr);
                    Log.Error(errorMsg);
                    return false;
                }
            }
            ...
            switch (cacheOp)
            {
                ...
                case CacheOperation.RemoveFromRoomCache:
                {
                    if (operation.Actors == null)
                    {
                        ...
                        {
                            var actors = this.GetActorsFromTraget(operation.Target, operation.ActorNr, 255, out errorMsg);
                            if (actors == null)
                            {
                                return false;
                            }
            ...
            errorMsg = string.Format("Unknown cache operation={0}.", operation.CacheOperation);
            return false;
        }
        ...
        void IPluginHost.HttpRequest(HttpRequest request)
        {
            ...
            try
            {
                ...
                this.EnqueuWebRequest(request, RequestRetryCount, webRequest, callback);
            }
            ...
            {
                Log.Error(string.Format("Exception calling Url:{0}, Exception Msg:{1}", request.Url, ex.Message), ex);
        ...
        private void EnqueuWebRequest(HttpRequest request, int RequestRetryCount, HttpWebRequest webRequest, HttpRequestQueueCallback callback)
        {
        ...
        HttpRequestQueue IPluginHttpUtilClient.HttpRequestQueue { get { return this.httpRequestQueue; } }

        int IPluginHttpUtilClient.HttpQueueRequestTimeout { get { return this.httpQueueRequestTimeout; } }

        IExtendedFiber IPluginHttpUtilClient.ExecutionFiber { get { return this.ExecutionFiber; } }
        ILogger IPluginHttpUtilClient.Log { get { return Log; } }
        ...
        private void SafeOneTimeTimerCallback(CallInfo info, Action callback)
        {
            ...
            try
            {
                ...
                string errorMsg;
                if (needCheck && !((ICallInfo)info).StrictModeCheck(out errorMsg))
                {
                    ...
                    info.Fail(string.Format("SafeOneTimeTimerCallback: {0}", errorMsg));
                }
            }
            catch (Exception e)
            {
        ...
        private void AddPredefinedHttpHeaders(HttpRequest request, HttpWebRequest webRequest)
        {
            foreach (var kv in request.Headers)
            {
                ...
                {
                    Log.Warn(string.Format("Header add exception:'{0}' with value '{1}'. Game:{2}. Excpetion Msg:{3}",
                        kv.Key, kv.Value, this, e.Message), e);
                }
            }
        }
        ...
        private bool ApplyRestrictedHeader(string key, string value, HttpWebRequest webRequest)
        {
            switch (key)
            {
                ...
                case "Proxy-Connection":
                    ...
                    return true;
                default:
                    return WebHeaderCollection.IsRestricted(key); // should return false
            }
            return true;
        }
            ...
            foreach (var kv in request.CustomHeaders)
            {
                ...
                catch (Exception e)
                {
                    Log.Warn(this.customHeaderExceptionLogGuard,
                        string.Format("Custom header add exception:'{0}' with value '{1}'. Game:{2}. Exception Msg:{3}",
                        kv.Key, kv.Value, this, e.Message), e);
                }
            }
        }
        ...
        private void HttpRequestCallback(HttpRequest request, HttpRequestQueueResultCode result, AsyncHttpRequest httpRequest, object state, ICallInfo info)
        {
            ...
            try
            {
                ...
                {
                    var infoTypeName = info.GetType().ToString();
                    this.Plugin.ReportError(ErrorCodes.MissingCallProcessing, null, infoTypeName);
        ...
        private IEnumerable<Actor> GetActorsFromTraget(byte target, int senderActor, byte targetGroup, out string errorMsg)
        {
            ...
            switch (target)
            {
                ...
                default:
                    ...
                    errorMsg = string.Format("Unknown target {0} specified in BroadcastEvent", target);
                    return null;
            }
            errorMsg = string.Empty;
            return actors;
        ...
        protected override void HandleCreateGameOperation(HivePeer peer, SendParameters sendParameters, JoinGameRequest createGameRequest)
        {
            ...
            if (this.pluginWrapper.ErrorPlugin != null)
            {
                Log.ErrorFormat("Game {0} is created with ErrorPlugin. message:{1}", this.Name, this.pluginWrapper.ErrorPlugin.Message);
            }
        ...
        private bool CreateGamePlugin(HivePeer peer, SendParameters sendParameters, JoinGameRequest joinGameRequest)
        {
            ...
            if (this.Plugin == null)
            {
                var errorMsg = string.Format("Failed to craete plugin '{0}'", pluginName);
        ...
        {
            var pluginInstance = new PluginInstance { Plugin = new PluginBase(), Version = new EnvironmentVersion() };
            ...
            RequestHandler handler = () =>
            {
                ...
                catch (Exception e)
                {
                    Log.Error(raiseEventExceptionLogGuard, $"Exception in RaiseEventHandler:Request:{raiseEventRequest.DumpRequest()}, " +
                              $"Actors:{this.ActorsManager.DumpActors()}, MCId:{this.MasterClientId}, SupressRoomEvents:{this.SuppressRoomEvents}", e);
        ...
        protected override void HandleSetPropertiesOperation(HivePeer peer, SetPropertiesRequest request, SendParameters sendParameters)
        {
            ...
            RequestHandler handler = () =>
            {
                ...
                {
                    if (request.OldPropertyValues != null)
                    {
        ...
        private void CallPluginOnLeaveIfJoinFailed(byte reason, HivePeer peer, JoinGameRequest request)
        {
            ...
            var leaveRequest = GetMockLeaveRequest(peer, actor != null ? actor.ActorNr : -1, false);
