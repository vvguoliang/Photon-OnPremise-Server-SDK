...
namespace Photon.Common.Authentication
{
    public class AuthTokenFactory
    {
        ...
        public bool DecryptAuthenticationTokenV1(string authTokenEncrypted, out AuthenticationToken authToken, out string errorMsg)
        {
            ...
            {
                errorMsg = string.Format("DecryptAuthenticationToken failed: Exception Msg:{0}", ex.Message);
        ...
        public bool DecryptAuthenticationTokenV2(string authTokenEncrypted, out AuthenticationToken authToken, out string errorMsg)
        {
            ...
            try
            {
                ...
                switch (tokenData[0])
                {
                    ...
                    default:
                        errorMsg = string.Format("Unknown version of token: {0}", tokenData[0]);
            ...
            {
                errorMsg = string.Format("DecryptAuthenticationTokenV2 failed: Exception Msg:{0}", ex.Message);
                return false;
            }
        }
        ...
        public bool DecryptAuthenticationTokenBynary(byte[] authTokenEncrypted, int offset, int len, out AuthenticationToken authToken, out string errorMsg)
        {
        ...
        private byte[] DecryptData(byte[] data, int offset, int len, out string errorMsg)
        {
            ...
            {
                errorMsg = "Incomming token data does not contain correct HMAC";
                return null;
            }
            errorMsg = string.Empty;
            // offset == 1. we skip version
            return this.decryptor.DecryptBufferWithIV(data, offset + 1, len - Encryptor.HMAC_SIZE - 1);
        }
        ...
        {
            return string.Format("{0}_{1}", Guid.NewGuid(), DateTime.UtcNow.ToString(CultureInfo.InvariantCulture));
        }
