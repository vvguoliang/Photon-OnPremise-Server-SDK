...
namespace Photon.Common.LoadBalancer.Prediction
{
    public class LoadStatsCollector
    {
        ...
        public bool SaveToFile(string dir, string fileName)
        {
            ...
            {
                log.WarnFormat("Error during saving of connfig. {0}", fullName);
                return false;
            }
            return true;
        }
        ...
        private void Serialize(XmlWriter writer)
        {
            ...
            this.SerializeControlers(writer);
            writer.WriteEndElement();
        }

        private void SerializeControlers(XmlWriter writer)
        {
            writer.WriteStartElement("FeedbackControllers");
            this.SerializeControler(writer);
            writer.WriteEndElement();
        }

        private void SerializeControler(XmlWriter writer)
        {
