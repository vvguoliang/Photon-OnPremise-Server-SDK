...
namespace Photon.LoadBalancing.GameServer
{
    ...
    public class Game : HiveHostGame
    {
        ...
        private readonly int ErrorsCountToInitiateUpdate = GameServerSettings.Default.ErrorsCountToInitiateUpdate;

        public virtual RoomCacheBase GameCache { get { return this.Application.GameCache; } }
        ...
        protected override void OnGamePropertiesChanged(SetPropertiesRequest request)
        {
            if (Log.IsDebugEnabled)
            {
                Log.DebugFormat("MaxPlayer={0}, IsOpen={1}, IsVisible={2}, #LobbyProperties={3}, #GameProperties={4}",
                    ...
                    request.newLobbyProperties == null ? 0 : request.newLobbyProperties.Count(),
                    request.newGameProperties == null ? 0 : request.newGameProperties.Count);
            }
                ...
                if (this.GameCache.TryGetRoomReference(this.Name, null, out RoomReference room))
                {
                    ...
                    if (this.removalStartTimestamp == DateTime.MinValue)// that means that we did not start removal
                    {
                        Log.Warn($"Game with 0 Actors is in cache:'{this.roomCache.GetDebugString(this.Name)}'." +
                                 ...
                                 $"IsDiposed:{this.IsDisposed}, IsClosed:{this.IsClosed}, EmptyRoomLiveTime:{this.EmptyRoomLiveTime}," +
        ...
        private void CheckPeerStates()
        {
            ...
            try
            {
                ...
                var inactivityInterval = new TimeSpan(GameServerSettings.Default.DisconnectAsNoActivityInterval, 0, 0);
                var lastTouch = -1;
                ...
                foreach (var actor in this.ActiveActors)
                {
                    ...
                    else
                    {
                        ...
                        {
                            var msg = string.Format("Check Peer State: Peer last touch {0} exceeds {1}. Removing peer from room {2}. p:{3}",
                                lastTouch, this.LastTouchLimitMilliseconds, this.Name, peer);
                        ...
                        else if (lastTouch == -1)
                        {
                            switch (peer.CheckCount)
                            {
                                ...
                                {
                                    ++peer.CheckCount;
                                    int roundTripTime, roundTripVariance, numOfFailures;
                                    peer.GetStats(out roundTripTime, out roundTripVariance, out numOfFailures);
                                    if (roundTripTime == -1)
                                    {
                                    ...
                                    var msg = string.Format("Check Peer State: Peer last touch is -1. rtt:{1}, rtv:{2}, numOfFailures:{3} p:'{0}'",
                                        peer, roundTripTime, roundTripVariance, numOfFailures);
                        ...
                        else
                        {
                            if (Log.IsDebugEnabled)
                            {
                                var msg = string.Format("Peer is active. LastTouchLimitMilliseconds: {0}, lastTouch={1}, Room: {2}. p:{3}",
                                    this.LastTouchLimitMilliseconds, lastTouch, this.Name, peer);
