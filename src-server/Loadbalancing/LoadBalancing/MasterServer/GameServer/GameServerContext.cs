...
namespace Photon.LoadBalancing.MasterServer.GameServer
{
    public class GameServerContext : IComparable<GameServerContext>
    {
        ...
        public GameServerAddressInfo AddressInfo { get; private set; }
        ...
        public byte LoadBalancerPriority { get; private set; }

        public int PeerCount { get; private set; }

        public List<byte> SupportedProtocols { get; private set; }

        public LogCountGuard OnServerDiconnectLogGuard { get; } = new LogCountGuard(new TimeSpan(0, 1, 0));
        #endregion
        ...
        {
            return string.Format("[key:{0}, serverId:{1}]", this.Key, this.ServerId);
        }
        #endregion
        ...
        {
            this.replicationFinishCheck = this.Peer.RequestFiber.Schedule(this.ReplicationFinishCheck, MasterServerSettings.Default.ReplicationFinishCheckInterval);
        }
        ...
        private void ReplicationFinishCheck()
        {
            ...
            if (this.replicationAssistant != null && !this.replicationAssistant.IsReplicationComplete)
            {
                log.WarnFormat("Replication stopped by timeout in {0} seconds after start. context:{1}", 
                    MasterServerSettings.Default.ReplicationFinishCheckInterval / 1000, this);
