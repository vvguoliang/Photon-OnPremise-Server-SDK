...
namespace Photon.LoadBalancing.MasterServer.Lobby
{
    ...
    public class AppLobby
    {
        ...
        public void UpdateLobbyLimits(bool? gameListUseLegacyLobbies, int? gameListLimit, int? gameListLimitUpdates, int? gameListLimitSqlFilterResults)
        {
            if (log.IsDebugEnabled)
            {
                log.DebugFormat("UpdateLobbyLimits ({4}/{5}), GameListUseLegacyLobbies {0}, GameListLimit {1}, GameListLimitUpdates {2}, GameListLimitSqlFilterResults {3}",
                    gameListUseLegacyLobbies, gameListLimit, gameListLimitUpdates, gameListLimitSqlFilterResults, LobbyName, LobbyType);
            }

            this.ExecutionFiber.Enqueue(() => GameList.UpdateLobbyLimits(gameListUseLegacyLobbies, gameListLimit, gameListLimitUpdates, gameListLimitSqlFilterResults));
        }

        public void UpdateMatchmakingStoredProcedure(string matchmakingStoredProcedure)
        {
            if (LobbyType == AppLobbyType.SqlLobby)
            {
                if (log.IsDebugEnabled)
                {
                    log.DebugFormat("UpdateMatchmakingStoredProcedure ({0}/{1}):\n###\n{2}\n###\n", LobbyName, LobbyType, matchmakingStoredProcedure);
                }
                this.ExecutionFiber.Enqueue(() => ((SqlGameList)GameList).UpdateMatchmakingStoredProcedure(matchmakingStoredProcedure));
            }
        }
        ...
        {
            // validate operation
            var operation = new JoinGameRequest(peer.Protocol, operationRequest, peer.UserId);
            OperationResponse response;
            if (OperationHelper.ValidateOperation(operation, log, out response) == false)
            {
                return response;
            ...
            // add peer to game
            return AddPlayerToGameAndGenerateResponse(peer, operation, gameState);
        }
